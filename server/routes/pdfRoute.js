...
router.post("/upload", uploadMiddleware.single("pdfFile"), async (req, res) => {
  ...
  if (isNaN(userId)) {
    return res.status(400).json({ message: "Invalid userId. It must be an integer." });
  }
  ...
  try {
    ...
    // Send response with pdf data and sourceId
    res.status(200).json({ pdf, sourceId });
  } catch (error) {
...
// List PDFs route
// List PDFs route with userId passed as a query parameter
router.get("/list", async (req, res) => {
  const userId = parseInt(req.query.userId, 10); // Convert to integer if needed
  if (isNaN(userId)) {
    return res.status(400).json({ message: "Invalid userId. It must be an integer." });
  }
